...
source_chord = [
    root,
    evans.PitchClassSet([root]).transpose(-1).invert(0).transpose(-7.5)[0],
    evans.PitchClassSet([root]).invert(0).transpose(3.5).transpose(0.5).invert(0).transpose(-6)[0],
    evans.PitchClassSet([root]).transpose(-1)[0],
    evans.PitchClassSet([root]).invert(0).transpose(3.5).invert(0).transpose(1.5)[0],
    evans.PitchClassSet([root]).invert(0).transpose(3.5)[0],
]
...
for tone in source_chord:
    out.append(
        evans.PitchClassSet(
            [
                tone,
                evans.PitchClassSet([tone]).transpose(-1).invert(0).transpose(-7.5)[0],
                evans.PitchClassSet([tone]).invert(0).transpose(3.5).transpose(0.5).invert(0).transpose(-6)[0],
                evans.PitchClassSet([tone]).transpose(-1)[0],
                evans.PitchClassSet([tone]).invert(0).transpose(3.5).invert(0).transpose(1.5)[0],
                evans.PitchClassSet([tone]).invert(0).transpose(3.5)[0],
            ]
        )
    )
...
for network in out:
    sub_network = evans.KlumpenhouwerNetwork(
        [
            network,
            network.transpose(-1).invert(0).transpose(-7.5),
            network.invert(0).transpose(3.5).transpose(0.5).invert(0).transpose(-6),
            network.transpose(-1),
            network.invert(0).transpose(3.5).invert(0).transpose(1.5),
            network.invert(0).transpose(3.5),
        ]
    )
...
for i, network in enumerate(meta_network):
    ...
    if i == 0:
        ...
        permutations = [[0, 1, 2, 3, 4, 5], [1, 0, 3, 2, 5, 4], [0, 1, 2, 3, 4, 5], [1, 0, 3, 2, 5, 4], [0, 1, 2, 3, 4, 5], [1, 0, 3, 2, 5, 4]]
        ...
        permutations = [[0, 1, 2, 3, 4, 5], [1, 3, 0, 4, 5, 2], [3, 4, 1, 5, 2, 0], [4, 5, 3, 2, 0, 1], [5, 2, 4, 0, 1, 3], [2, 0, 5, 1, 3, 4]]
        for segment, permutation in zip(network, permutations):
            permuted_network.append(segment.permute(permutation))
        permuted_network = abjad.sequence.rotate(permuted_network, 0-i)
        permuted_meta_network.append(permuted_network)
        ...
        permutations = [[0, 1, 2, 3, 4, 5], [5, 3, 4, 1, 0, 2], [2, 1, 0, 3, 5, 4], [4, 3, 5, 1, 2, 0], [0, 1, 2, 3, 4, 5], [5, 3, 4, 1, 0, 2]]
        for segment, permutation in zip(network, permutations):
            permuted_network.append(segment.permute(permutation))
        permuted_network = abjad.sequence.rotate(permuted_network, 0-i)
        permuted_meta_network.append(permuted_network)
